...
## Iteration 55 — Final pre-launch polish (June 2026)
Fixes (restore point: tag iter54-restore-point): (1) 71% completion ROOT CAUSE: completion_checklist counted optional items (home_city, prompt) in the percentage; now % = REQUIRED items only (photos2, bio40, email, current city, 3+ interests — city/interests promoted to required) and optional items labelled "(optional)"; profile-setup adds required Current city input + bio>=40 + 3-interest gates so onboarding presents all required fields. (2) Pending queue BUG: counts/queries used status "Pending" but data stores "Pending Review"; added NEEDS_REVIEW_STATUSES=["Pending Review","In Review"] single source of truth used by dashboard KPI, action-required list, status panel and the Pending queue mapping. (3) Notifications Centre: GET/POST /api/control/notifications-centre(+/unread-count,/read) derived from REAL records (verifications needing review, annual review due, more-info, open reports, email-provider failure); per-admin persistent read state in db.admin_notification_reads; frontend /control/alerts + sidebar "Notifications (n)" badge (60s poll), old push screen renamed "Broadcasts". (4) Professional header restructured: 56px avatar + display name + profession + Verified badge, actions row below (Edit Profile/View Profile/Verification); "Open to paying/Free advice/Last 4h" chips removed. (5) Web verification polish: 640px container, section headings, responsive @640 breakpoint, secondary buttons; CUSTOMER_WEB_BASE_URL env preferred for customer-facing links (email interstitial + desktop links) — owner must attach orrbbit.com custom domain + have support set the env before links switch (no fake domain swap).
Tests: tests/test_iter55_polish.py 10/10; 56 green across iter54/52/50/control suites; testing agent iteration_55 all 5 frontend features green. QA NOTE: the control override env was temporarily commented during UI testing and RESTORED after (verify /control browser again points at production).
needs_retesting: false

## Iteration 56 — Final correction pass (June 2026)
(1) DOMAIN FINDING: orrbbit.com currently routes to a DIFFERENT, OLDER deployment (its API root says "Orrbbit API is running" — a string that never existed in this repo; /api/email/verify there is 404). Defaulting customer links to orrbbit.com would break every email link, so links remain on the working platform host until: owner re-points orrbbit.com custom domain to THIS production deployment AND Emergent Support sets CUSTOMER_WEB_BASE_URL=https://orrbbit.com on the production backend. Code already prefers CUSTOMER_WEB_BASE_URL everywhere (email_templates PUBLIC_BASE_URL + verification_web) — switching is config-only, no code change needed later. (2) Profile Setup now includes full optional profile fields: Country, From/home city, one conversation prompt (testIDs setup-country, setup-home-city, setup-prompt-q/a) in addition to required Display name/city/bio40/interests3/photos2; optional fields don't block completion. (3) Professional Radar (I Can Help): profile name/photo header REMOVED from radar top; RadarView now gets onFilters/onRadiusPress/filterCount (Filters + Radius buttons match People radar); payment/fresh filters restored via a bottom filter sheet (testIDs filter-pay-*, filter-fresh); Edit Profile/View Profile/Verification actions moved into the requests bottom sheet header. Verified via Playwright: Filters + "Radius: 250m" visible, header gone.
Tests: iter54+iter55 suites 20/20 green after changes.
needs_retesting: false
