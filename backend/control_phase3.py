"""IntroU Control Centre — Phase 3 modules.
...
PLANS = [
    ...
    {"key": "intro_plus", "name": "IntroU Plus", "price": None, "features": ["100m radius", "Priority pings"]},
    {"key": "intro_professional", "name": "IntroU Professional", "price": None, "features": ["500m radius", "Professional tools", "Verified badge priority"]},
]
...
async def billing_integration(admin: dict = Depends(require_perm("payments"))):
    ...
    return {
        ...
        "security": "Card details, security codes and payment credentials are never stored in the IntroU database. The provider remains the financial source of truth; IntroU stores only references and synced status.",
    }
...
async def _run_ai(report_label: str, metrics: dict, settings: dict) -> str:
    ...
    chat = LlmChat(
        ...
        system_message=("You are the analytics director for IntroU, a proximity-based social + professional networking app. "
...
async def export_ai_insight(report_id: str, format: str = "pdf", admin: dict = Depends(require_perm("ai"))):
    ...
    pdf = _make_pdf(f"IntroU AI Insight — {r['label']}", r["content"] + f"\n\n---\n{r['disclaimer']}\nGenerated {r['created_at']} by {r['generated_by']} ({r['mode']} mode)")
...
def _rows_to_file(rows: list, columns: list, name: str, format: str) -> Response:
    ...
    if format == "pdf":
        ...
        return Response(content=_make_pdf(f"IntroU Export — {name}", " | ".join(columns) + "\n" + text), media_type="application/pdf",
...
async def export_entity(entity: str, request: Request, format: str = "csv",
    ...
    return _rows_to_file(rows, cols, f"introu-{entity}-{mode}", format)
...
async def backup_log(backup_id: str, admin: dict = Depends(require_super_admin())):
    ...
    return Response(content=f"IntroU backup {backup_id}\nstatus: {b['status']}\npath: {b['path']}\n\n{b.get('log', '')}",
